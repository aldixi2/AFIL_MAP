...
def main():
    ...
    for cod, ag in agregados.items():
        ...
        # Fechas robustas: soporta fechas ya convertidas, texto y seriales de Excel.
        ...
        for col_fecha in ('FECHA_NAC', 'FECHA_AFIL'):
            if col_fecha in detalle_df.columns:
                detalle_df[col_fecha] = detalle_df[col_fecha].apply(convertir_fecha).dt.strftime('%d/%m/%Y')
    ...
    # ---- Manifest ----
    mensual_2026 = Counter()
    for ag in agregados.values():
        for mes, cantidad in ag["por_mes_afil_2026"].items():
            mensual_2026[int(mes)] += int(cantidad)

    manifest = {
        "generado": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "total_afiliados": len(vistos),
        "por_mes_afil_2026": {str(k): int(v) for k, v in sorted(mensual_2026.items())},
